...
class VerificationCode(db.Model):
    ...
    code_type = db.Column(db.String(20), nullable=False, index=True)  # email_verify, password_reset, login_verify, phone_verify
    
    # 用户信息
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # 可为空，支持注册前验证
    ...
    # 发送信息
    send_method = db.Column(db.String(10), default='email', nullable=False)  # email, sms
    send_count = db.Column(db.Integer, default=1, nullable=False)  # 发送次数
    last_send_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    ...
    # 索引
    __table_args__ = (
        db.Index('idx_verification_codes_email_type', 'email', 'code_type'),
        db.Index('idx_verification_codes_phone_type', 'phone', 'code_type'),
