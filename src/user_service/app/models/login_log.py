...
class LoginLog(Base):
    ...
    user_id = Column(Integer, ForeignKey('users.id'), nullable=True)  # 可为空，支持记录失败的登录尝试
    username = Column(String(80), nullable=True)  # 记录尝试登录的用户名
    email = Column(String(120), nullable=True)  # 记录尝试登录的邮箱
    
    # 登录信息
    login_type = Column(String(20), nullable=False, default='web')  # web, mobile, api
    login_method = Column(String(20), nullable=False, default='password')  # password, oauth, sms
    ...
    user_agent = Column(String(500), nullable=True)  # 使用 String 替代 Text
    device_info = Column(String(1000), nullable=True)  # 设备信息，使用 String 替代 JSON
    location_info = Column(String(500), nullable=True)  # 地理位置信息，使用 String 替代 JSON
    
    # 安全信息
    is_suspicious = Column(Boolean, default=False, nullable=False)  # 是否可疑登录
    risk_score = Column(Integer, default=0, nullable=False)  # 风险评分 0-100
    security_flags = Column(String(1000), nullable=True)  # 安全标记，使用 String 替代 JSON
    
    # 会话信息
    session_id = Column(String(128), nullable=True)
    token_id = Column(String(128), nullable=True)  # JWT token ID
    ...
    __table_args__ = (
        ...
        Index('idx_login_logs_ip_created', 'ip_address', 'created_at'),
        Index('idx_login_logs_suspicious', 'is_suspicious', 'created_at'),
    )
